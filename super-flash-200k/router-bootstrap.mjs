...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    // DSH 0.1.2-rc.1 replaced Session.events with snapshotEvents(); using the
    // current API keeps the "first durable tool/call promotes the full catalog"
    // promotion working instead of silently never firing.
    ...
    // Context-compiler tools are part of the super-flash-200k memory surface
    // and must stay available BEFORE the first tool/call promotion too.
    ...
    return {
      ...
      sections,
      contexts: [],
      tools: assembled.tools.filter((tool) => core.has(tool.name) || compilerTools.has(tool.name)),
    }
  })
